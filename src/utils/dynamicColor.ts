...
export function getTrackCoverArt(track?: Partial<Track> | null): string {
  ...
  // 1. If explicit cover art is already present (e.g. from uploaded audio thumbnail or edited in Studio)
  if (track.coverArt && track.coverArt !== DEFAULT_FALLBACK_ART && !track.coverArt.includes('placeholder')) {
    return getResolvedCoverArt(track.coverArt);
  }

  // 2. If known track ID has pre-assigned cover
  if (track.id && KNOWN_TRACK_COVERS[track.id]) {
    return KNOWN_TRACK_COVERS[track.id];
  }

  // 3. Fallback matching by title for tracks without cover art
...
export function getResolvedCoverArt(url?: string | null): string {
  ...
  if (url.startsWith('linear-gradient') || url.startsWith('radial-gradient') || url.startsWith('blob:') || url.startsWith('data:')) {
    return url;
  }
