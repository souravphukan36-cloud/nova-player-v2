...
export const LibraryTab: React.FC<LibraryTabProps> = ({ initialSubTab }) => {
  ...
  const downloadedTracks = sortedTracks.filter(t => downloadedTrackIds.has(t.id) || t.isDownloaded);
    ...
    <div className="space-y-4 pb-36 px-5 select-none animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between py-1 text-xs text-white/60">
        <span>
          ...
          {activeSubTab === 'downloaded' && `${downloadedTracks.length} Offline Tracks (Saved on Device)`}
        ...
        {(activeSubTab === 'songs' || activeSubTab === 'downloaded') && (
          <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <div className="relative">
                    ...
                    {trackMenuId === track.id && (
                      <div className="absolute right-0 top-8 z-30 w-48 rounded-2xl bg-neutral-900 border border-white/15 p-1.5 shadow-2xl text-xs animate-in zoom-in-95">
                        <button
                          onClick={() => {
                            playNext(track);
                            setTrackMenuId(null);
                        ...
                        <button
                          ...
                          className="w-full text-left px-3 py-2 rounded-xl hover:bg-white/10 text-white flex items-center justify-between"
                        >
                          <span>{downloadedTrackIds.has(track.id) || track.isDownloaded ? 'Redownload Track' : 'Download Offline'}</span>
      ...
      {/* 2. DOWNLOADED SUBTAB (Offline Tracks) */}
        ...
        <div className="space-y-3">
          {downloadedTracks.length > 0 && (
            <div className="flex items-center justify-between p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span className="text-emerald-300 font-semibold">{downloadedTracks.length} tracks available offline</span>
              </div>
              <button
                onClick={() => playTrack(downloadedTracks[0], downloadedTracks)}
                className="px-3.5 py-1.5 rounded-full bg-emerald-500 text-black font-extrabold text-[11px] active:scale-95 shadow flex items-center gap-1.5"
              >
          ...
          {downloadedTracks.length === 0 ? (
            <div className="text-center py-16 px-4 rounded-3xl bg-white/5 border border-white/10 space-y-3">
              <Download className="w-12 h-12 mx-auto text-white/30" />
              <h3 className="text-sm font-bold text-white">No Downloaded Songs Yet</h3>
              <p className="text-xs text-white/50 max-w-xs mx-auto">
                Download songs to listen offline anytime without internet or mobile data.
              </p>
              <button
                onClick={() => setActiveSubTab('songs')}
                className="px-4 py-2 rounded-full text-black font-bold text-xs shadow transition-transform active:scale-95"
                style={{ backgroundColor: settings.accentColor }}
              >
